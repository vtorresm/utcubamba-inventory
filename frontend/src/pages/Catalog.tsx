...
const Catalog: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (filters.administrationRoute) {
      result = result.filter(med => med.administrationRoute === filters.administrationRoute)
    }

    if (filters.medicalCondition) {
      result = result.filter(med => med.medicalCondition === filters.medicalCondition)
    }

    if (filters.availability) {
      result = result.filter(med => med.availability === filters.availability)
    }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h2 className="text-3xl font-bold mb-6">Medication Catalog</h2>
      <div className="mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredMedications.map(med => (
          <div key={med.id} className="bg-white rounded-lg shadow-md overflow-hidden transition-transform duration-300 hover:scale-105">
            <div className="p-6">
              <h3 className="text-xl font-semibold mb-2">{med.name}</h3>
              <p className="text-gray-600 mb-4">{med.category}</p>
              <div className="mb-4">
                <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                  med.availability === 'In Stock' ? 'bg-green-100 text-green-800' :
                  med.availability === 'Limited' ? 'bg-yellow-100 text-yellow-800' :
                  'bg-red-100 text-red-800'
                }`}>
                  {med.availability}
                </span>
              </div>
              <p className="text-gray-700 mb-2"><strong>Administration:</strong> {med.administrationRoute}</p>
              <p className="text-gray-700 mb-2"><strong>Condition:</strong> {med.medicalCondition}</p>
              <p className="text-xl font-bold text-blue-600">${med.price.toFixed(2)}</p>
            </div>
          </div>
        ))}
      </div>
